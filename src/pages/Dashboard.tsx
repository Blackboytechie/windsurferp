...
export default function Dashboard() {
  ...
  `)
      ...
      if (!topProducts) {
        ...
        return; // Handle the case where no products are returned
      }

      const productSales = topProducts.reduce<Record<string, { product: Product; total_quantity: number }>>(
        (acc, item) => {
          const productId = item.product_id;
          if (!acc[productId]) {
            acc[productId] = {
              product: item.product as Product, // Type assertion since Supabase returns it as any
              total_quantity: 0,
            };
          }
